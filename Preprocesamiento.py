...
def graficar_seniales(senial_original, senial_procesada, fs1,fs2, a):
    tiempo = np.arange(len(senial_original)) / fs1
    tiempo2 = np.arange(len(senial_procesada)) / fs2

    senial_original_en_frecuencia = fft(senial_original)
    senial_procesada_en_frecuencia  = fft(senial_procesada)
    frecuencia = fftfreq(len(senial_original), 1 / fs1)
    frecuencia2 = fftfreq(len(senial_procesada),1/ fs2)

    fig, axs = plt.subplots(2, 2, figsize=(12, 8))
    plt.suptitle("Comparación entre señal sin procesar y señal procesada")
    axs[0, 0].plot(tiempo, senial_original)
    axs[0, 0].set_title("Señal original en el dominio del tiempo")
    axs[0, 0].set_xlabel("Tiempo [s]")
    axs[0, 0].set_ylabel("Amplitud")

    axs[0, 1].plot(frecuencia, np.abs(senial_original_en_frecuencia))
    axs[0, 1].set_title("Señal original en el dominio de la frecuencia")
    axs[0, 1].set_xlabel("Frecuencia [Hz]")
    axs[0, 1].set_ylabel("Amplitud")

    axs[1, 0].plot(tiempo2, senial_procesada)
    axs[1, 0].set_title("Señal" + a +" en el dominio del tiempo")
    axs[1, 0].set_xlabel("Tiempo [s]")
    axs[1, 0].set_ylabel("Amplitud")

    axs[1, 1].plot(frecuencia2, np.abs(senial_procesada_en_frecuencia ))
    axs[1, 1].set_title("Señal" + a + " en el dominio de la frecuencia")
...
def segmentacion_del_audio(nombre_del_archivo_de_audio_a_filtrar_Segmentar_ventanear, duracion_del_frame=20, minima_duracion_del_audio_en_seg=2, frames_totales=100):
    ...
    graficar_seniales(senial_original_a_filtrar_segmentar_ventanear, senial_filtrada_con_preenfasis, sr_a_filtrar_segmentar_ventanear,sr_a_filtrar_segmentar_ventanear," filtrada con Preénfasis")
    ...
    # Graficar el frame antes y después de aplicar la ventana de Hamming y la ventana de Hamming sola
    plt.figure(figsize=(12, 8))
    
    # Agregar un gráfico para visualizar la ventana de Hamming
    plt.subplot(2, 2, 1)
    plt.plot(ventana_de_hamming)
    plt.title("Ventana de Hamming")
    plt.xlabel("Muestras")
    plt.ylabel("Amplitud")

    plt.subplot(2, 2, 2)
    plt.plot(frame_antes_hamming)
    plt.title("Frame antes de aplicar la ventana de Hamming")
    plt.xlabel("Muestras")
    plt.ylabel("Amplitud")

    plt.subplot(2, 2, 3)
    plt.plot(frame_despues_hamming)
    plt.title("Frame después de aplicar la ventana de Hamming")
    plt.xlabel("Muestras")
    plt.ylabel("Amplitud")
...
graficar_seniales(muestras_ruido, muestras_filtradas, fs_original, fs_objetivo, " filtrada")
...
graficar_seniales(audio_silencios,audio_sin_silencios,sr_silencios,sr_silencios," sin silencios grandes")
...
graficar_seniales(audio_a_normalizar, audio_normalizado, sr_a_normalizar, sr_a_normalizar , " normalizada")
...
# Cargar los frames desde un archivo NumPy
frames_cargados_con_preenfasis = load_frames_from_file(ruta_de_archivo_de_salida_segmentado_con_preenfasis)
print(frames_cargados_con_preenfasis.shape)


frames_cargados_sin_preenfasis = load_frames_from_file(ruta_de_archivo_de_salida_segmentado_sin_preenfasis)
print(frames_cargados_sin_preenfasis.shape)
